...
function setupThree() {
  ...
  gui.add(handData, 'x', -1, 1).name('Hand X').step(0.01).listen();
  gui.add(handData, 'y', -1, 1).name('Hand Y').step(0.01).listen();
  gui.add(handData, 'isPinching').name('Pinch').listen();
}

function updateThree() {

  // update the picking ray with the camera and pointer position
  const finger = new THREE.Vector2(handData.x, handData.y);
