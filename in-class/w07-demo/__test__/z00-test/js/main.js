...
function setupThree() {
  ...
  gui.add(params, 'particles').listen();
...
function updateThree() {
  ...
  for (let i = 0; i < particles.length; i++) {
    let p = particles[i];
    colorArray[i * 3 + 0] = 1;
    colorArray[i * 3 + 1] = random(1);
    colorArray[i * 3 + 2] = 0.0;
  }

  params.particles = particles.length;
...
function getPoints() {
  // vertices with length of 50000
  const vertices = new Float32Array(50000 * 3);
